...
public class Program
{
    ...
    public void StartMenu()
    {
        ...
        Console.Write("\n[-] Name building types: Сrc32, XxHash128\n");
        ...
        switch (args[0])
        {
            ...
            case "6.1":
                ...
                break;
            case "0":
                if (mappix.settingsProfile.RenameClasses == true && mappix.nameBuilderRule.Type == BuildType.Empty) 
                { 
                    Thread.Sleep(300); Console.WriteLine("Can't to obfuscate with this parameters, use: 6.1 Crc32 or XxHash128");
                } 
                if (Directory.Exists(path) == true) { Console.Clear(); StartObfuscating(); return; }
                else 
                { 
                    Thread.Sleep(300);  Console.WriteLine("Directory not exists!"); 
                }
                break;
            case "profiler":
                Console.Clear();
